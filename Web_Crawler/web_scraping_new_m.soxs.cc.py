...
base_url = "https://m.soxs.cc/MoShiZhiWeiWoDuZun/3026905.html"
x = 0
...
while True:
    ...
    title = remove_space_line(
        doc.xpath("//body[@id='chapter']//h1[1]")[0].text_content())
    content = remove_space_line(
        doc.xpath("//body[@id='chapter']/div[5]")[0].text_content())
    ...
    if base_url.find('htm') == -1:
        ...
        if doc.xpath("//body[@id='chapter']//h1[1]"):
            next_click_text = remove_space_line(
                doc.xpath("(//div[@class='pager']//a)[3]")[0].text_content())
        ...
        # if '/' in doc.xpath("//div[@class='pager']//a)[3]")[1].get('href'):
        if True:
            url_temp = base_url.split('/')[0] + '//' + base_url.split('/')[2]
            # base_url = 'https://m.xbiquge.cc' + doc.xpath("//td[@class='next']//a")[1].get('href')
            print(doc.xpath("//div[@class='pager']//a)[3]"))
            break
            base_url = url_temp + \
                doc.xpath("//div[@class='pager']//a)[3]")[1].get('href')
            nomore_page_close(base_url)
        else:
            url_temp = base_url.split(
                '/')[0]+'//'+base_url.split('/')[2]+'/'+base_url.split('/')[3]+'/'
