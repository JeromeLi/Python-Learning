...
base_url = 'https://www.biquge5200.cc/97_97184/163356295.html'
...
while True:
    ...
    try:
        ...
        html_title_str = driver.find_element_by_xpath("//div[@class='bookname']//h1[1]").text
        # html_str = driver.find_element_by_xpath("//div[@class='content']").text
        html_str = driver.find_element_by_xpath("//div[@id='content']").text
        # html_str = html_str.replace('喜欢万族之劫请大家收藏：(m.soxscc.net)万族之劫搜小说更新速度最快。','')
        save = SaveText(filename, html_title_str)
        save.title()
        save = SaveText(filename, html_str)
        save.article()
        print ("continue...")
        driver.find_element_by_xpath("//div[@id='wrapper']/div[4]/div[1]/div[2]/div[1]/a[4][contains(text(),'下一章')]").click()
        # driver.find_element_by_xpath("//body[@id='chapter']/div[7]/a[3][contains(text(),'下一章')]").click()
        # next_page = driver.find_element_by_xpath("//body[@id='chapter']/div[7]/a[3]").get_text()
        # if next_page == "下一章":
        #     driver.find_element_by_xpath("//body[@id='chapter']/div[7]/a[3]").click()
        # sleep(1000)
